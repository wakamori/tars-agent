...
let worker: Matter.Body;
let robot: Matter.Body;
let safetyBarriers: Matter.Body[] = [];
let isRunning = false;
let isPaused = false;
let startTime = 0;
...
function createFactoryFloor(): void {
  const wallOptions = {
    isStatic: true,
    render: {
      fillStyle: "#333333",
...
// Handle collisions
function handleCollision(event: { pairs: Matter.Pair[] }): void {
  ...
  for (const pair of pairs) {
    ...
    if (
      (bodyA.label === "worker" && bodyB.label === "robot") ||
...
function isBetween(
  ...
  const barrierDist1 = Math.sqrt(
    Math.pow(barrierPos.x - pos1.x, 2) + Math.pow(barrierPos.y - pos1.y, 2)
  );
  const barrierDist2 = Math.sqrt(
    Math.pow(barrierPos.x - pos2.x, 2) + Math.pow(barrierPos.y - pos2.y, 2)
  );
...
async function analyzeWithAI(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      const mockData = (await mockResponse.json()) as AgentResponse;
...
function processAnalysisResults(data: AgentResponse | null): void {
  if (!data) {
  ...
  logThinking(
    `検出: ${entityCount}個のエンティティ, リスク最大値: ${maxRisk}, 優先度: ${intervention_decision?.priority || 0}`,
    "success"
  );
}

// Display agent's thinking process (Self-Inquiry)
function displayThinkingProcess(selfInquiry: SelfInquiry): void {
  ...
  // Memory connections
  if (
    selfInquiry.memory_connections &&
    selfInquiry.memory_connections.length > 0
  ) {
    html += '<div class="inquiry-section">';
    html += "<h4>🧠 記憶との関連</h4>";
    html += '<ul class="inquiry-list">';
  ...
  // Accident scenarios
  if (
    selfInquiry.accident_scenarios &&
    selfInquiry.accident_scenarios.length > 0
  ) {
    html += '<div class="inquiry-section">';
...
// Display discovered patterns
function displayDiscoveredPatterns(patterns: DiscoveredPattern[]): void {
...
function applyInterventionDecision(decision: InterventionDecision): void {
  ...
  logThinking(
    `優先度 ${decision.priority}/10: ${action.type} を実施`,
    "success"
  );
}
...
function createSafetyBarrier(x: number, y: number): void {
...
function updatePreventedCount(): void {
  const elem = document.getElementById("preventedCount");
  if (elem) {
    elem.textContent = String(preventedAccidents);
  }
}

function updateRiskLevel(
  level: string,
  risk: "safe" | "low" | "high" = "safe"
): void {
...
function updateConfidence(confidence: number): void {
  const percent = Math.round(confidence * 100);
  const elem = document.getElementById("aiConfidence");
  if (elem) {
...
function clearWarnings(): void {
  const elem = document.getElementById("warningsList");
  if (elem) {
    elem.innerHTML = '<p class="no-warnings">現在、警告はありません</p>';
  }
}

function clearInterventions(): void {
  const elem = document.getElementById("interventionsList");
  if (elem) {
    elem.innerHTML = '<p class="no-interventions">介入なし</p>';
  }
}

type LogType = "info" | "success" | "warning" | "error";

function logThinking(message: string, type: LogType = "info"): void {
...
function updateTime(): void {
  ...
  if (elem) {
    elem.textContent = `時間: ${elapsed.toFixed(1)}s`;
  }
...
function getInterventionIcon(
  type: InterventionAction["type"]
): string {
  const icons: Record<InterventionAction["type"], string> = {
    barrier: "🛡️",
    slowdown: "🐢",
    alert: "🚨",
    evacuation: "🏃",
...
function getInterventionLabel(
  ...
  const labels: Record<InterventionAction["type"], string> = {
    barrier: "安全バリア配置",
    slowdown: "ロボット減速",
    alert: "警告表示",
    evacuation: "緊急退避",
...
async function fetchMemoryStream(): Promise<void> {
  try {
...
function displayMemoryStream(data: MemoryResponse): void {
  ...
  if (memories && memories.length > 0) {
    ...
    recentMemories.forEach((mem) => {
      const importanceStars = "⭐".repeat(
        Math.min(Math.floor(mem.importance / 2), 5)
      );
      const time = mem.timestamp
        ? new Date(mem.timestamp).toLocaleTimeString("ja-JP")
        : "N/A";
